...
/* ---- data: [name, color, pts [cost $, success %], label anchor] ----
   Labels are colored to their series and sit only in verified-clear space
   (tools/audit.mjs now fails the build on any text×text, text×line or
   text×dot contact, every frame, every ratio) */
const SERIES = [
  { name: 'fable 5.1', color: '#b3a24a', pts: [[9.7, 73], [6.3, 72.4], [4.3, 69], [2.6, 67.4]], lab: [85, 44, 'start'] },
  { name: 'opus 5', color: '#6f9cb8', pts: [[7.2, 69.9], [6.0, 69], [3.5, 66.3], [2.3, 64]], lab: [240, 52, 'start'] },
  { name: 'gpt-5.6 sol', color: '#c96f5f', pts: [[5.3, 67], [2.9, 63.6], [2.2, 63.1]], lab: [340, 160, 'start'] },
  { name: 'grok 4.6', color: '#aab2ba', pts: [[1.9, 70.5], [1.4, 69.3], [0.7, 66.7], [0.35, 60.6]], lab: [578, 74, 'end'] },
  { name: 'sonnet 5', color: '#9a8fd6', pts: [[3.4, 61], [1.9, 58.2], [1.2, 56.3], [0.7, 47.1]], lab: [640, 330, 'end'] },
  { name: 'gemini 3.7 flash', color: '#7f8fd0', pts: [[1.0, 61.1], [0.75, 58.7], [0.5, 53.6]], lab: [640, 300, 'end'] },
  { name: 'composer 2.5', color: '#b58a5f', pts: [[0.75, 64.3], [0.4, 58.9], [0.15, 55.5]], lab: [640, 265, 'end'] },
]
const US = { name: 'superbot', color: 'var(--accent)', pts: [[0.75, 72.1], [0.55, 70.6], [0.3, 67.8]], lab: [760, 27, 'end'], stat: [796, 51, 'end'] }
...
export const benchmarkSpot = {
  ...
  audit: {
    ...
    cutT: CUT_T,
    lines: [
      { at: STAT_AT, cps: 22, text: '72.1% · $0.30', sel: '[data-type="stat"]', coverAt: CUT_T },
  ...
  tick(t, p) {
    ...
    P.typer.stat.run(t >= STAT_AT, '72.1% · $0.30', 22, t - STAT_AT)
  ...
  const rival = (s, i) => `
    <g class="bm-rival" data-riv="${i}">
      ...
      <text x="${s.lab[0]}" y="${s.lab[1]}" text-anchor="${s.lab[2]}" class="bm-lab" data-riv-lab="${i}"
        style="opacity:0;fill:${s.color}">${s.name}</text>
    </g>`
  const ours = `
  ...
  return `
    <svg class="bm-svg" viewBox="0 0 800 450" aria-hidden="true">
      <g data-axes style="opacity:0">
        <rect x="70" y="4" width="152" height="24" rx="12" fill="none" stroke="var(--accent)" stroke-width="1"/>
        <text x="146" y="20" text-anchor="middle" font-size="12" fill="var(--accent)">cost per task</text>
        <text x="262" y="20" text-anchor="middle" font-size="12" fill="var(--muted)" opacity="0.55">time per task</text>
        ${ticks.map((p) => `
