/* frame-render — export a spot as an mp4 via deterministic frame stepping.
   Same virtual clock as tools/audit.mjs: steps the loop at `fps`, captures
   each frame as a PNG, then pipes them through ffmpeg (yuv420p, faststart —
   safe for ad-platform uploads). One frame per sample, zero drift.
     node tools/render.mjs --out cheaper            → .tmp/render/cheaper.mp4
     node tools/render.mjs --ratio 9:16 --fps 30    → vertical
     node tools/render.mjs --t0 10 --t1 13.5        → clip only          */
...
const name = arg('out', `${spotName}-${ratio.replace(':', 'x')}${t0 > 0 || t1 < DUR ? `-clip${t0}-${t1}` : ''}`)
...
console.log(`${name}.mp4 — ${frames} frames @ ${fps}fps (${ratio}) in ${((Date.now() - t0ms) / 1000).toFixed(0)}s → ${out}`)
